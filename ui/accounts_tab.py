            ...
            if hasattr(app, "control_frame"):
                def on_move_complete():
                    ...
                    def schedule_lobbies():
                        ...
                        self._logManager.add_log("🎯 Move completed → Starting MakeLobbiesAndSearchGame...")

                        def run_lobbies():
                            try:
                                current_app = self.winfo_toplevel()
                                if (hasattr(current_app, "main_menu") and
                                    hasattr(current_app.main_menu, "_lobbyManager")):
                                    current_app.main_menu._lobbyManager.MakeLobbiesAndSearchGame()
                                    self._logManager.add_log("✅ AUTO via main_menu._lobbyManager!")
                                elif hasattr(current_app, "main_menu"):
                                    ...
                                    lobby = LobbyManager()
                                    lobby.MakeLobbiesAndSearchGame()
                                    self._logManager.add_log("✅ AUTO via direct LobbyManager!")
                            except Exception as e:
                                self._logManager.add_log(f"❌ Lobbies error: {e}")

                        threading.Thread(target=run_lobbies, daemon=True).start()
