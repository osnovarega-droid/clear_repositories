        ...
        while True:
            ...
            if len(cs2_pids) >= 4:
                if start_detect_time is None:
                    ...
                    self.logManager.add_log("🚀 Таймер истёк → MOVE ALL CS WINDOWS")
                    self.move_all_cs_windows()
                    #self.logManager.add_log("✅ Windows moved! Calling callback...")
                    ...
                    if callback:
                        try:
                            if cancel_check and cancel_check():
                                ...
                                return
                            callback()
                           # self.logManager.add_log("✅ Callback executed!")
                        except Exception as e:
