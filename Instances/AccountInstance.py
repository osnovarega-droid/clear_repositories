...
class Account:
    ...
    def parse_current_level(self):
        ...
        try:
            ...
            if level > 0:
                ...
                print(f"✅ [{self.login}] Спарсено: Lv{level} XP{xp}")
                return True
            else:
    ...
    def _fetch_steam_html(self, steam, url_suffix="gcpd/730/?tab=matchmaking"):
        """Вспомогательный метод для Steam HTML"""
    ...
    def ProcessWindowsAfterCS(self, steamPid):
        """
        Закрывает ТОЛЬКО Steam UI окна. ПОЛНАЯ защита CS2!
        """
        try:
            parent = psutil.Process(steamPid)
            ...
            for pid in all_pids:
                try:
                    ...
                    for hwnd in windows:
                        ...
                        if ("Counter-Strike 2" in window_title or 
                            "cs2.exe" in window_title.lower() or 
                            self.login in window_title or
                            "[FSN FREE]" in window_title):
                            print(f"🛡️ CS2 окно защищено: {window_title[:50]}...")
                            continue
                            
                        # Steam UI только
                        if any(x in window_title for x in ["Steam", "Friends", "Login", "Sign in", "Store", "Special Offers"]):
                            if win32gui.IsWindowVisible(hwnd) and win32gui.IsWindowEnabled(hwnd):
                                win32gui.PostMessage(hwnd, win32con.WM_CLOSE, 0, 0)
                                print(f"🪟 Закрыто Steam UI: {window_title[:50]}...")
                        else:
                            print(f"⚠️ Пропущено: {window_title[:30]}...")
